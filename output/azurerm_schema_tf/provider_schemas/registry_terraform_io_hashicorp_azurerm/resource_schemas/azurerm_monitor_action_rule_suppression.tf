resource "azurerm_monitor_action_rule_suppression" "name" {
  ...
  suppression block "list"  (Required){
    recurrence_type = string (Required)

    schedule block "list"  (Optional){
      end_date_utc = string (Required)
      start_date_utc = string (Required)
      recurrence_monthly = ['set', 'number'] (Optional)
      recurrence_weekly = ['set', 'string'] (Optional)
    }
  }
