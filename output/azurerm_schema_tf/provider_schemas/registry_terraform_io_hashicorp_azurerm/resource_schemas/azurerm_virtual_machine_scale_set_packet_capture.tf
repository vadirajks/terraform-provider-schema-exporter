resource "azurerm_virtual_machine_scale_set_packet_capture" "name" {
  ...
  machine_scope block "list" (Optional) {
    exclude_instance_ids = ['list', 'string'] (Optional)
    include_instance_ids = ['list', 'string'] (Optional)
  }
