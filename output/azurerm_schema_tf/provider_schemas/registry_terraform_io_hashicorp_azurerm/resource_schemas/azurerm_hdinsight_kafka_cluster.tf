resource "azurerm_hdinsight_kafka_cluster" "name" {
  ...
  rest_proxy block "list" (Optional) {
    security_group_id = string (Required)
    security_group_name = string (Required)
  }
  ...
  roles block "list" (Required) {
    ...
    kafka_management_node block "list" (Optional) {
      vm_size = string (Required)
      password = string (Optional)
      ssh_keys = ['set', 'string'] (Optional)
      subnet_id = string (Optional)
    ...
    worker_node block "list" (Required) {
      number_of_disks_per_node = number (Required)
