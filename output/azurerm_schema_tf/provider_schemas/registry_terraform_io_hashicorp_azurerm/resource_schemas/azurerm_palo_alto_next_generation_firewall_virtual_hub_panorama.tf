resource "azurerm_palo_alto_next_generation_firewall_virtual_hub_panorama" "name" {
  location = string (Required)
  name = string (Required)
  panorama_base64_config = string (Required)
  resource_group_name = string (Required)
  id = string (Optional, Computed)
  marketplace_offer_id = string (Optional)
  panorama = ['list', ['object', {'device_group_name': 'string', 'host_name': 'string', 'name': 'string', 'panorama_server_1': 'string', 'panorama_server_2': 'string', 'template_name': 'string', 'virtual_machine_ssh_key': 'string'}]] (Computed)
  ...
  network_profile block "list" (Required) {
    network_virtual_appliance_id = string (Required)
    ...
    trusted_subnet_id = string (Computed)
    untrusted_subnet_id = string (Computed)
  }
