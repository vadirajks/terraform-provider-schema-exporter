resource "azurerm_hdinsight_hadoop_cluster" "name" {
  ...
  roles block "list" (Required) {
    ...
    edge_node block "list" (Optional) {
      target_instance_count = number (Required)
      vm_size = string (Required)

      https_endpoints block "list" (Optional) {
        access_modes = ['list', 'string'] (Optional)
        destination_port = number (Optional)
        disable_gateway_auth = bool (Optional)
        private_ip_address = string (Optional)
        sub_domain_suffix = string (Optional)
      }

      install_script_action block "list" (Required) {
        name = string (Required)
        uri = string (Required)
        parameters = string (Optional)
      }

      uninstall_script_actions block "list" (Optional) {
        name = string (Required)
        uri = string (Required)
        parameters = string (Optional)
      }
