...
# --- Helper functions for cleaning descriptions ---
...
def traverse_and_clean(node):
    ...
    if isinstance(node, dict):
        for key, value in node.items():
            # Only process 'description' keys that are strings
...
base_folder = os.path.join(args.base_dir, f"{args.provider}_schema_yaml", "provider_schemas", sanitize(provider_key))
os.makedirs(base_folder, exist_ok=True)

# Write YAML files for all nested schemas
for section, content in provider_schema.items():
    ...
    if isinstance(content, dict):
        for item, value in content.items():
            ...
            # --- ADDED ---
            # Clean all 'description' fields in the object before writing
            traverse_and_clean(value)
            # --- END ADDED ---
            
            item_file = os.path.join(section_folder, f"{sanitize(item)}.yaml")
            with open(item_file, "w") as f:
                # --- MODIFIED ---
                # Added width=float('inf') to prevent re-wrapping long lines
                yaml.dump(value, f, sort_keys=False, width=float('inf'))
                # --- END MODIFIED ---

print(f"✅ Completed YAML export for provider: {args.provider}")
